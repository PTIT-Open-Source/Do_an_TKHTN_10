...
#define BUTTON_GPIO    GPIO_NUM_4   // D4 = GPIO4

static uint8_t message_tid = 0;

static uint8_t dev_uuid[16];
static uint8_t bind_index = 0;
static bool binding_in_progress = false;  // Đang thực hiện bind
...
static void send_onoff_set_to_node(esp_ble_mesh_node_info_t *node);
static uint32_t button_press_count = 0;
static uint32_t last_button_state = 1;  // Giả sử button bình thường là HIGH
...
static void button_init(void)
{
    gpio_config_t io_conf = {
        ...
        .pull_up_en = GPIO_PULLUP_ENABLE,    // Bật pull-up nếu cảm biến không tự kéo lên
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,      // Chỉ cần polling
    };
    gpio_config(&io_conf);
}
...
void button_task(void *arg)
{
    ...
    gpio_pullup_en(BUTTON_GPIO); // Nếu sensor là dạng open-drain cần kéo lên

    while (1) {
        uint32_t button_state = gpio_get_level(BUTTON_GPIO);

        if (button_state != last_button_state) {
            vTaskDelay(pdMS_TO_TICKS(10)); // debounce ~10ms
            button_state = gpio_get_level(BUTTON_GPIO); // đọc lại

            if (button_state != last_button_state) {
                last_button_state = button_state;

                if (button_state == 0) {  // Nếu phát hiện nhấn (tuỳ bạn xét LOW hay HIGH)
                    button_press_count++;

                    ESP_LOGI("BUTTON", "Button pressed count = %" PRIu32, button_press_count);


                    // Gửi OnOff dựa vào count
                    esp_ble_mesh_node_info_t *node = example_ble_mesh_get_node_info(0x0005); // địa chỉ unicast server
                    if (node) {
                        if (button_press_count % 2 == 1) {
                            // Bật LED
                            ESP_LOGI("BUTTON", "Send ON command");
                            send_onoff_set_to_node(node); // custom thêm tham số nếu cần
                        } else {
                            // Tắt LED
                            ESP_LOGI("BUTTON", "Send OFF command");
                            send_onoff_set_to_node(node); // custom thêm tham số nếu cần
                        }
                    }
                }
                                                 ...
                                                 esp_ble_mesh_model_t *model, uint32_t opcode)
{
    ...
    ESP_LOGI(TAG, "Sending to 0x%04x with TTL = %d", common->ctx.addr, common->ctx.send_ttl);
    return ESP_OK;
}
                               ...
                               uint16_t unicast, uint8_t elem_num, uint16_t net_idx)
{
    ...
    // common.ctx.app_idx = 0x0000;               // Nếu đây là AppKey đầu tiên
    // common.ctx.addr = unicast;                 // Địa chỉ của node vừa provision
    // common.msg_timeout = 0;
    // common.ctx.send_ttl = 3;    
    // // common.msg_role = ROLE_PROVISIONER;

    // set_state.model_app_bind.element_addr = unicast;
    // set_state.model_app_bind.model_app_idx = 0x0000;
    // set_state.model_app_bind.model_id = ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV;
    // set_state.model_app_bind.company_id = 0xFFFF;

    // err = esp_ble_mesh_config_client_set_state(&common, &set_state);
    // if (err != ESP_OK) {
    //     ESP_LOGE(TAG, "Failed to send AppKey Bind (err %d)", err);
    // } else {
    //     ESP_LOGI(TAG, "Sent AppKey Bind to node 0x%04x (Generic OnOff Client)", unicast);
    // }
    return ESP_OK;
}
...
static void send_onoff_set_to_node(esp_ble_mesh_node_info_t *node)
{
    ...
    for (int i = 0; i < node->elem_num; i++) {
        example_ble_mesh_set_msg_common(&common, node, onoff_client.model,
                                        ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK);
        common.ctx.addr = node->unicast + i;
        set_state.onoff_set.op_en = false;
        set_state.onoff_set.onoff = 1;
        set_state.onoff_set.tid = message_tid++;

        err = esp_ble_mesh_generic_client_set_state(&common, &set_state);
        if (err) {
            ESP_LOGE(TAG, "Send OnOff Set Unack failed to 0x%04x", common.ctx.addr);
        } else {
            ESP_LOGI(TAG, "Sent OnOff Set to element addr 0x%04x", common.ctx.addr);
        }
...
static void send_next_app_bind(esp_ble_mesh_node_info_t *node) {
    esp_ble_mesh_cfg_client_set_state_t set_state = {0};
    esp_ble_mesh_client_common_param_t common = {0};

    if (bind_index >= node->elem_num) {
        ESP_LOGI(TAG, "App Bind completed for node 0x%04x", node->unicast);
        bind_index = 0;
        binding_in_progress = false;

        // send_onoff_set_to_node(node);
        return;
    }
    ...
    set_state.model_app_bind.element_addr = node->unicast + bind_index;
    set_state.model_app_bind.model_app_idx = prov_key.app_idx;
    set_state.model_app_bind.model_id = ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_SRV;
    set_state.model_app_bind.company_id = ESP_BLE_MESH_CID_NVAL;

    if (esp_ble_mesh_config_client_set_state(&common, &set_state) == 0) {
        ESP_LOGI(TAG, "Sent App Bind to element %d (addr 0x%04x)", bind_index, set_state.model_app_bind.element_addr);
        binding_in_progress = true;
        // bind_index sẽ ++ sau khi xác nhận callback thành công
    } else {
        ESP_LOGE(TAG, "Failed to send App Bind to element %d", bind_index);
    }
}
                                              ...
                                              esp_ble_mesh_cfg_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD: {
            ...
            err = esp_ble_mesh_config_client_set_state(&common, &set_state);
            if (err) {
                ESP_LOGE(TAG, "%s: Config Model App Bind(Server) failed", __func__);
                return;
            }
            ESP_LOGI(TAG, "Sent App Bind to OnOff Server");
            break;
        }
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND: {
            ...
            // }
            // break;
            ESP_LOGI(TAG, "App Bind success for element %d", bind_index);
            bind_index++;
            send_next_app_bind(node);  // Gửi tiếp element tiếp theo
            break;
        }
        default:
            break;
