                                     ...
                                     esp_ble_mesh_msg_ctx_t *ctx, uint8_t onoff)
{
    ...
    uint16_t target_addr = ctx->recv_dst;

    if (ESP_BLE_MESH_ADDR_IS_UNICAST(target_addr)) {
        int index = target_addr - primary_addr;
        ESP_LOGI(TAG, "model addr = 0x%04x, primary = 0x%04x, index = %d", model->element->element_addr, primary_addr, index);
        ESP_LOGI(TAG, "Received msg for addr 0x%04x", target_addr);
        if (index >= 0 && index < elem_count) {
            board_led_operation(led_state[index].pin, onoff);
            ESP_LOGI(TAG, "LED[%d] (GPIO %d) set to %s",
                     index, led_state[index].pin, onoff ? "ON" : "OFF");
        } else {
            ESP_LOGW(TAG, "Unicast addr 0x%04x is out of element range", target_addr);
        }

    } else if (ESP_BLE_MESH_ADDR_IS_GROUP(target_addr)) {
        if (esp_ble_mesh_is_model_subscribed_to_group(model, target_addr)) {
            int index = model->element->element_addr - primary_addr;
            // int index = model->element - esp_ble_mesh_get_element(0);
            if (index >= 0 && index < elem_count) {
                board_led_operation(led_state[index].pin, onoff);
                ESP_LOGI(TAG, "Group addr LED[%d] (GPIO %d) set to %s",
                         index, led_state[index].pin, onoff ? "ON" : "OFF");
            }
        }

    } else if (target_addr == ESP_BLE_MESH_ADDR_ALL_NODES) {
        int index = model->element->element_addr - primary_addr;
        if (index >= 0 && index < elem_count) {
            board_led_operation(led_state[index].pin, onoff);
            ESP_LOGI(TAG, "Broadcast LED[%d] (GPIO %d) set to %s",
                     index, led_state[index].pin, onoff ? "ON" : "OFF");
        }
    }
}
...
{
    ESP_LOGI(TAG, "GEN_SERVER_CB: model addr=0x%04x, opcode=0x%04" PRIx32,
     param->model->element->element_addr, param->ctx.recv_op);
    ...
    case ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT:
        ESP_LOGI(TAG, "ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT");
         ESP_LOGI(TAG, "Received message from 0x%04x to 0x%04x with TTL %d",
